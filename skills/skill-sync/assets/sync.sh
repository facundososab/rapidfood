...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo "  --scope      Only sync specific scope (root, frontend, backend)"
            exit 0
            ;;
        *)
...
# Collect skills by scope (bash 3.x compatible: no associative arrays)
SCOPE_SKILLS_FILE=$(mktemp)

_scope_get() {
    grep "^$1=" "$SCOPE_SKILLS_FILE" 2>/dev/null | head -1 | sed "s/^$1=//"
}

_scope_set() {
    local _scope="$1" _value="$2" _tmp
    _tmp=$(mktemp)
    grep -v "^$_scope=" "$SCOPE_SKILLS_FILE" > "$_tmp" 2>/dev/null || true
    printf '%s=%s\n' "$_scope" "$_value" >> "$_tmp"
    mv "$_tmp" "$SCOPE_SKILLS_FILE"
}
...
while IFS= read -r skill_file; do
    ...
    for scope in "${scopes[@]}"; do
        ...
        existing=$(_scope_get "$scope")
        if [ -z "$existing" ]; then
            _scope_set "$scope" "$skill_name:$auto_invoke"
        else
...
while IFS= read -r scope; do
    scopes_sorted+=("$scope")
done < <(sed 's/=.*//' "$SCOPE_SKILLS_FILE" | sort -u)
...
|--------|-------|"
    ...
    IFS='|' read -ra skill_entries <<< "$(_scope_get "$scope")"
...
| $action | \`$skill_name\` |"
    ...
    if $DRY_RUN; then
        ...
        if grep -q "### Auto-invoke Skills" "$agents_path"; then
            ...
            # Insert after Skills Reference blockquote, or append at end if not found
            awk '
                /^>.*SKILL\.md\)$/ && !inserted {
                    print
                    getline
