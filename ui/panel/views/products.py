...
def form(request,product_id=None): return page(request,'products/form.html',{**_ctx(request),'product':get_client().get_product(product_id) if product_id else None})
def save(request,product_id=None):
 if request.method!='POST': return HttpResponseBadRequest()
 p=get_client().save_product({'id':product_id,'description':request.POST['description'],'category_id':request.POST['category_id'],'available':request.POST.get('available')=='on','price':request.POST.get('price') or None}); return redirect('product_detail',product_id=p.id)
